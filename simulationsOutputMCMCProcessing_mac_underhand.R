...
colnames(MCMC_Diagnostics_PSRF) <- c("ntraits", "rateMatrixCorrelation", "replicateNum", "TL_Point", "TL_UpperCI", "LL_Point", "LL_UpperCI")
MCMC_Diagnostics_Geweke <- matrix(ncol = 7)
colnames(MCMC_Diagnostics_Geweke) <- c("ntraits", "rateMatrixCorrelation", "replicateNum", "gTL1", "gTL2", "gLL1", "gLL2")
MCMC_Diagnostics_ESS <- matrix(ncol = 7)
colnames(MCMC_Diagnostics_ESS) <- c("ntraits", "rateMatrixCorrelation", "replicateNum", "essTL1", "essTL2", "essLL1", "essLL2")
# MCMC_Diagnostics_ASDSF <- matrix()
MCMC_Diagnostics_NodalFreqCorr <- matrix(ncol = 5)
colnames(MCMC_Diagnostics_NodalFreqCorr) <- c("ntraits", "rateMatrixCorrelation", "replicateNum", "correlations", "numBipartsNotInChain")

Tree_Dist <- matrix(ncol = 5)
colnames(Tree_Dist) <- c("ntraits", "rateMatrixCorrelation", "replicateNum", "rfDist", "kfDist")
MJCT_Compatibility <- matrix(ncol = 5)
colnames(MJCT_Compatibility) <- c("ntraits", "rateMatrixCorrelation", "replicateNum", "numInconsistencies", "conTreeResolution")
...
for (j in 1:6) {
  ...
  for(i in windowCodes[m]:(windowCodes[m])){
    ...
    png(paste0("/Users/nikolai/mvBM_sims_underhand/plots/", fileName, "_TraceMarginals.png"), width = 1920, height = 1080)
    par(mfrow = c(2,2))
    ...
    MCMC_Diagnostics_PSRF <- rbind(MCMC_Diagnostics_PSRF, c(ntraits, rateMatrixCorrelation, replicateNum, TLsPSRF, LLsPSRF))
    ...
    MCMC_Diagnostics_Geweke <- rbind(MCMC_Diagnostics_Geweke, c(ntraits, rateMatrixCorrelation, replicateNum, gTL1, gTL2, gLL1, gLL2))
    ...
    MCMC_Diagnostics_ESS <- rbind(MCMC_Diagnostics_ESS, c(ntraits, rateMatrixCorrelation, replicateNum, essTL1, essTL2, essLL1, essLL2))
    ...
    MJCT_Compatibility <- rbind(MJCT_Compatibility, c(ntraits, rateMatrixCorrelation, replicateNum, numInconsistensies, conTree$Nnode))
    
    rfDist <- dist.topo(x = trueTree, y = mccTree, method = "PH85")
    kfDist <- dist.topo(x = trueTree, y = mccTree, method = "score")
    Tree_Dist <- rbind(Tree_Dist, c(ntraits, rateMatrixCorrelation, replicateNum, rfDist, kfDist))
    ...
    MCMC_Diagnostics_NodalFreqCorr <- rbind(MCMC_Diagnostics_NodalFreqCorr, c(ntraits, rateMatrixCorrelation, replicateNum, corT1T2, numBipartsNotInChain1orChain2))
...
a <- bwplot(numCorrectNodes ~ ntraits, main = "Consistency with MJ-Consensus Tree -- 0.2 correlation", xlab = "Number of Traits Used in Simulation", 
            key = key, ylab = "Number of Nodes Identified in Strict Majority-Rule Consensus Tree", 
            data=MJCT_Compatibility[MJCT_Compatibility[,2] == "2",], ylim = c(0,25), pch="|", coef=0, panel=function(...){panel.hanoi(col="green", alpha = .7, ...); panel.bwplot(...)})
b <- bwplot(numInconsistencies ~ ntraits, data=MJCT_Compatibility[MJCT_Compatibility[,2] == "perfect",], ylim = c(0,30), pch="|", coef=0, 
            panel=function(...){panel.hanoi(col="red", alpha = .7, ...)})
a + as.layer(b)

a <- bwplot(numCorrectNodes ~ ntraits, main = "Consistency with MJ-Consensus Tree -- 0.4 correlation", xlab = "Number of Traits Used in Simulation", 
            key = key, ylab = "Number of Nodes Identified in Strict Majority-Rule Consensus Tree", 
            data=MJCT_Compatibility[MJCT_Compatibility[,2] == "4",], ylim = c(0,25), pch="|", coef=0, panel=function(...){panel.hanoi(col="green", alpha = .7, ...); panel.bwplot(...)})
b <- bwplot(numInconsistencies ~ ntraits, data=MJCT_Compatibility[MJCT_Compatibility[,2] == "interregional",], ylim = c(0,30), pch="|", coef=0, 
            panel=function(...){panel.hanoi(col="red", alpha = .7, ...)})
a + as.layer(b)

a <- bwplot(numCorrectNodes ~ ntraits, main = "Consistency with MJ-Consensus Tree -- 0.8 correlation", xlab = "Number of Traits Used in Simulation", 
            key = key, ylab = "Number of Nodes Identified in Strict Majority-Rule Consensus Tree", 
            data=MJCT_Compatibility[MJCT_Compatibility[,2] == "8",], ylim = c(0,25), pch="|", coef=0, panel=function(...){panel.hanoi(col="green", alpha = .7, ...); panel.bwplot(...)})
  ...
  for (k in 1:length(degreeCorrelation)) {
    ...
    for (j in 1:6) { #iterates over trait number
      ...
      for(i in m:m){ #iterates over replicates
        ...
        MCMC_Diagnostics_Distances[rowCounter, 2,] <- rep(rateMatrixCorrelation, 26668)
        ...
        png(paste0("/Users/nikolai/mvBM_sims_underhand/plots/treeDist/", fileName, "_TreeDistTraceMarginals.png"), width = 1920, height = 1080)
        par(mfrow = c(2,2))
  ...
  for (k in 1:length(degreeCorrelation)) {
    ...
    for (j in 1:1) { #iterates over trait number
      ...
      for(i in m:m){ #iterates over replicates
        ...
        MCMC_Diagnostics_Distances_prior[rowCounter, 2,] <- rep(rateMatrixCorrelation, 26668)
...
for (k in 1:length(degreeMisspecification)) {
  ...
  for (j in 1:1) { #iterates over trait number
    ...
    for(i in 1:100){ #iterates over replicates
      ...
      ntraits <- traitNumIter[j]
      rateMatrixCorrelation <- degreeMisspecification[k]
      replicateNum <- i
      ...
      MCMC_Diagnostics_Distances_prior[i, 2,] <- rep(rateMatrixCorrelation, 26668)
...
colnames(means) <- c("ntraits", "rateMatrixCorrelation", "replicateNum", "idk", "rfDist", "kfDist")
...
legend(54, 55,levels(means$rateMatrixCorrelation),col=1:length(means$rateMatrixCorrelation),pch=1)
...
legend(54, 3.1,levels(means$rateMatrixCorrelation),col=1:length(means$rateMatrixCorrelation),pch=1)
...
colnames(sds) <- c("ntraits", "rateMatrixCorrelation", "replicateNum", "idk", "rfDist", "kfDist")
plot(as.numeric(as.character(sds$ntraits)), as.numeric(as.character(sds$rfDist)), col = sds$rateMatrixCorrelation, 
...
legend(54, 2.1,levels(sds$rateMatrixCorrelation),col=1:length(sds$rateMatrixCorrelation),pch=1)

plot(as.numeric(as.character(sds$ntraits)), as.numeric(as.character(sds$kfDist)), col = sds$rateMatrixCorrelation, 
...
legend(54, 3.2,levels(sds$rateMatrixCorrelation),col=1:length(sds$rateMatrixCorrelation),pch=1)
...
colnames(MCMC_Diagnostics_Distances) <- c("ntraits", "rateMatrixCorrelation", "replicateNum", "runNum", "rfDist", "kfDist")
