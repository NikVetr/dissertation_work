...
foreach(m=1:500, .packages = c("ape", "coda", "rwty", "phangorn", "lattice", "latticeExtra")) %dopar% {
  
  traitNumIter <- c(2,4,8,16,32,57)
  traitNumIter <- rev(traitNumIter)
  degreeMisspecification <- c("perfect", "interregional", "chimpHuman")
  noisyness <- "noiseless"
  # degreeMisspecification <- c("2", "4", "6", "8")
  # degreeMisspecification <- c("9")
  # degreeMisspecification <- c("3")
...
traitNumIter <- c(2,4,8,16,32,57)
degreeMisspecification <- c("perfect", "interregional", "chimpHuman")
# degreeMisspecification <- c("2")

for (k in 1:length(degreeMisspecification)) { #degree misspecification
  ...
  for (j in 1:length(traitNumIter)) { #num traits
    ...
    for(i in 1:100){ #replicate
      ...
      # nums[i] <- (mean(posteriorBiparts[posteriorBiparts[,1] > 0.9 & posteriorBiparts[,1] < 1,3]))
  ...
  # save(posteriorBipartsReduced, file = paste0("bipartitionPosteriors/posteriorBipartsReduced_PB_128Traits_noError0.", rateMatrixMisspecification))
  save(posteriorBipartsReduced, file = paste0("bipartitionPosteriors/posteriorBipartsReduced_Howells_allTraits_", rateMatrixMisspecification))
}
...
setwd(dir = "/Volumes/2TB/500/full/mvBM_sims_Howells_500/")
degreeMisspecification <- c("perfect", "interregional", "chimpHuman")
misspecification <- "perfect"
load(file = paste0("bipartitionPosteriors/posteriorBipartsReduced_Howells_allTraits_", misspecification))
intervalSize <- 0.1
...
png(filename = "~/mvbm_manuscript/figures/calibrationCurveHowells.png", width = 900, height = 600)
par(mfrow = c(1,1), mar = c(3,3,3,3))
plot(expected, observed, ylab = "Observed Frequency", xlab = "Inferred Probability", xlim = c(0,1), ylim = c(0,1),
     # main = paste0("Calibration Curve for Binned (", intervalSize, ") Nodal Posterior Probabilities\nPure-Birth Trees, Meas. Error, Corr = 0.", misspecification, ", All Traits Analyses"))
     # main = paste0("Calibration Curve for Binned (", intervalSize, ") Nodal Posterior Probabilities\nPure-Birth Trees, ", tools::toTitleCase(noisyness)))
     main = paste0("Calibration Curve for Binned (", intervalSize, ") Nodal Posterior Probabilities\nHowells' Trees"))
    abline(a = 0, b = 1, col = 2, lwd = 2)
lines(expected, observed)
text(misspecification, x = expected[length(expected)]+0.04, y = observed[length(expected)])

for(cs in 2:length(degreeMisspecification)){
  misspecification <- degreeMisspecification[cs]
  load(file = paste0("bipartitionPosteriors/posteriorBipartsReduced_Howells_allTraits_", misspecification))
  ...
  lines(expected, observed)
  if(misspecification == 9){misspecification <- 99}
  text(substr(misspecification, 0,5), x = expected[length(expected)]+0.04, y = observed[length(expected)])
}

#getting the uvBM analyses in there too
for(cs in 1:length(degreeMisspecification)){
  misspecification <- degreeMisspecification[cs]
  load(file = paste0("/Volumes/2TB/500/full/uvBM_sims_Howells_500/bipartitionPosteriors/posteriorBipartsReduced_Howells_allTraits_", misspecification))
  file = paste0("bipartitionPosteriors/posteriorBipartsReduced_Howells_allTraits_", misspecification)
  expected <- rep(0, length(breaks)-1)
  observed <- rep(0, length(breaks)-1)
  for(i in 1:length(breaks)-1){
    inInterval <- posteriorBipartsReduced[posteriorBipartsReduced[,1] > breaks[i] & posteriorBipartsReduced[,1] < breaks[i+1],]
  ...
  # if(misspecification == 9){misspecification <- 99}
  if(misspecification != "perfect"){
    text(substr(misspecification, 0,5), x = expected[length(expected)]+0.04, y = observed[length(expected)], col = "gray50")} else {
      text(misspecification, x = expected[length(expected)]+0.04, y = observed[length(expected)], col = "gray50")    
  }
...
for(i in 1:length(breaks)){
  inInterval <- posteriorBipartsReduced[posteriorBipartsReduced[,1] > breaks[i] - intervalSize/2 & posteriorBipartsReduced[,1] < breaks[i] + intervalSize/2,]
  expected[i] <- mean(as.numeric(inInterval[,1]))
  observed[i] <- mean(as.numeric(inInterval[,3]))
}
par(mfrow = c(1,1))
...
for (k in 1:3) {
  ...
  for (j in 1:6) {
    ...
    for(i in 1:100){
      ...
      fileName <- paste0("simulations_", ntraits, "traits_perfectRateMatrix_", rateMatrixMisspecification, "misspecification_", "replicate_", replicateNum)
      load(file = paste0("bipartitionPosteriors/", fileName, "_bipartitionProbs"))
...
for (k in 1:length(degreeMisspecification)) {
  ...
  for (j in 1:6) {
    ...
    for(i in 1:100){
      ...
      rateMatrixMisspecification <- degreeMisspecification[k]
      replicateNum <- i
      cat(c(ntraits, rateMatrixMisspecification, replicateNum, "... \n"))
      
      percLLs[counter, 1:3] <- percTLs[counter, 1:3] <- percPats[counter, 1:3] <- percTermBLs[counter, 1:3] <- percColless[counter, 1:3] <- c(ntraits, rateMatrixMisspecification, replicateNum)
...
for (k in 1:1) {
  ...
  for (j in 1:1) {
    ...
    for(i in 1:100){
      ...
      percLLsPrior[counter, 1:3] <- percTLsPrior[counter, 1:3] <- percPatsPrior[counter, 1:3] <- percTermBLsPrior[counter, 1:3] <- percCollessPrior[counter, 1:3] <- c(ntraits, rateMatrixMisspecification, replicateNum)
...
for (k in 1:1) {
  ...
  for (j in 1:1) {
    ...
    for(i in 1:100){
      ...
      percTLsPrior[counter, 1:3] <- c(ntraits, rateMatrixMisspecification, replicateNum)
...
for (j in 1:6) {
  ...
  for (k in 1:length(degreeMisspecification)) {
    ...
    ntraits <- traitNumIter[j]
    rateMatrixMisspecification <- degreeMisspecification[k]
    hist(as.numeric(percPats[percPats[,1] == ntraits & percPats[,2] == rateMatrixMisspecification, 4:435]),
         xlim = c(0,1), breaks = 100, main = paste0(rateMatrixMisspecification, " RMS, ", ntraits, " traits"), freq = F, xlab = "", ylab = "")
...
for (j in 1:6) {
  ...
  for (k in 1:length(degreeMisspecification)) {
    ...
    ntraits <- traitNumIter[j]
    rateMatrixMisspecification <- degreeMisspecification[k]
    hist(as.numeric(percTLs[percTLs[,1] == ntraits & percTLs[,2] == rateMatrixMisspecification, 4]),
         xlim = c(0,1), breaks = 5, main = paste0(rateMatrixMisspecification, " RMS, ", ntraits, " traits"), freq = F, xlab = "", ylab = "")
...
for (j in 1:6) {
  ...
  for (k in 1:length(degreeMisspecification)) {
    ...
    ntraits <- traitNumIter[j]
    rateMatrixMisspecification <- degreeMisspecification[k]
    hist(as.numeric(percColless[percColless[,1] == ntraits & percColless[,2] == rateMatrixMisspecification, 4]),
         xlim = c(0,1), breaks = 10, main = paste0(rateMatrixMisspecification, " RMS, ", ntraits, " traits"), freq = F, xlab = "", ylab = "")
...
for (j in 1:6) {
  ...
  for (k in 1:length(degreeMisspecification)) {
    ...
    ntraits <- traitNumIter[j]
    rateMatrixMisspecification <- degreeMisspecification[k]
    hist(as.numeric(percTermBLs[percTermBLs[,1] == ntraits & percTermBLs[,2] == rateMatrixMisspecification, 4:33]),
         xlim = c(0,1), breaks = 30, main = paste0(rateMatrixMisspecification, " RMS, ", ntraits, " traits"), freq = F, xlab = "", ylab = "")
