setwd("/Volumes/2TB/mvBM_sims_PB_noiseless")
setwd("/Volumes/2TB/mvBM_sims_PB_noiseless_8tips/")
setwd("/Volumes/2TB/500/mvBM_sims_PB_noiseless_500")
...
noisyness <- "noisy"
setwd("/Volumes/2TB/500/mvBM_sims_PB_noisy_500//")
  ...
  for(i in 1:50){
    ...
    for (j in 1:length(traitNumIter)) {
      ...
      for (l in 1:nrun) {
        ...
        if(remote){
          cat("## Specify the Network Drive\nsetwd(\"/Volumes/2TB/P_test\")\n\n")
        }
