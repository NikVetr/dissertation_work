...
data_subset <- c(95,99,100)[3]
sex <- "females"
collapseHomo = T
load(paste0("/Users/nikolai/data/Harvati_", ifelse(collapseHomo, "no", ""),"HomoPops_", sex,"_PCA", data_subset,"_PopData"))
numPops <- length(pops)
...
save(divergenceCodedTraits, file = paste0("/Users/nikolai/data/Harvati_", ifelse(collapseHomo, "no", ""),"HomoPops_", sex,"_PCA", data_subset,"_DivCodedTraits"))

load(paste0("/Users/nikolai/data/Harvati_", ifelse(collapseHomo, "no", ""),"HomoPops_", "males","_PCA", data_subset,"_DivCodedTraits"))
male_divergence_coded_traits <- divergenceCodedTraits
load(paste0("/Users/nikolai/data/Harvati_", ifelse(collapseHomo, "no", ""),"HomoPops_", "females","_PCA", data_subset,"_DivCodedTraits"))
...
writeDist(distmat, file = paste0("output/parsDistances_empirical_",
                                 ifelse(collapseHomo, "no", ""), "HomoPops_", "PCA", data_subset, "_divergenceCoding.dist"))
...
write.tree(bestTrees, paste0("output/maximumParsimonyTree_empirical_",
                             ifelse(collapseHomo, "no", ""), "HomoPops_", "PCA", data_subset, "_divergenceCoding.txt"))
...
if(homopops){
  ...
  females <- undoMeansNexus(fileString = paste0("data2_neanF/Harvati_noHomoPops_females_PCA", percPCA, ".nex"))
  males <- undoMeansNexus(fileString = paste0("data2_neanF/Harvati_noHomoPops_males_PCA", percPCA, ".nex"))
}
...
write.tree(njTree, file = paste0("output/empirical_PCA", percPCA ,"_neighborJoining.txt"))
write.tree(upgmaTree, file = paste0("output/empirical_PCA", percPCA ,"_upgma.txt"))
