...
logLL_bivProb_optim_multithresh <- function(par_to_opt, dat){ #TMB package may help with speedup
  ...
  #but just fix it to the appropriate beta of the marginal LKJ
  vectorized <- T
  univMeans <- T
  univThresh <- T
  treeReg <- T
  ...
  if(par_type == "mean"){
    ...
    # regTerm <- -sum(dnorm(x = log(cor_beta_shape_params), mean = lognormal_prior_on_beta_shape_params_for_corrs[1], sd = lognormal_prior_on_beta_shape_params_for_corrs[1], log = T))
    logLL <- 0
    regTerm <- 0
  }
...
logLL_bivProb_optim_multithresh(rep(100,5), dat)
...
for(i in 1:nrounds){
  ...
  if(ncore == 1){
    for(par_to_opt_ind in param_ord){
      ...
      lower = c(rep(-Inf, nTaxa * d_traits), 0.01, rep(-1, choose(d_traits, 2)), rep(0, (max_thresholds_count-1) * d_traits), c(-Inf, 0, 0, 0))[par_to_opt_ind]
    ...
    if(mclapply_over_foreach){
      ...
      for(par_to_opt_ind in 1:(length(param_ord) / ncore)){
        ...
        dat <- lapply(1:ncore, function(core) list(dat_orig, k_par[[core]], prunes, pops, n_thresh))
  ...
  est_means_PCs <- PC_Scores[,1:which.max(cumsum(L))] / L[1:which.max(cumsum(L))]^0.5
...
curr_est_index <- max(sapply(1:length(list.files("data")), function(prms) as.numeric(strsplit(list.files("data")[prms], split = "_")[[1]][4]))) + 1
