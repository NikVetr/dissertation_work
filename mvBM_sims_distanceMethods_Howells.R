...
distMatrixNJDists <- matrix(nrow = length(degreeMisspecification)*length(traitNumIter)*100, ncol = 6)
distMatrixUPGMADists <- matrix(nrow = length(degreeMisspecification)*length(traitNumIter)*100, ncol = 6)
...
for (k in 1:length(degreeMisspecification)) {
  ...
  for (j in 1:length(traitNumIter)) { #iterates over trait number
    ...
    for(i in 1:100){ #iterates over replicates
      ...
      distMatrixNJDists[rowCounter,] <- c(ntraits, rateMatrixMisspecification, "NJ", replicateNum, rfDistNJ, kfDistNJ)
      distMatrixUPGMADists[rowCounter,] <- c(ntraits, rateMatrixMisspecification, "UPGMA", replicateNum, rfDistUPGMA, kfDistUPGMA)
    }
  }
}
...
colnames(distMatrixUPGMADists) <- c("ntraits", "rateMatrixMisspecification", "type", "replicateNum", "rfDist", "kfDist")
colnames(distMatrixNJDists) <- c("ntraits", "rateMatrixMisspecification", "type", "replicateNum", "rfDist", "kfDist")
