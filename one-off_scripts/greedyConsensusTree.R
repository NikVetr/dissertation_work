...
trees <-c(read.tree("output/harvati_noPCA_c1.trees"), read.tree("output/harvati_noPCA_c2.trees"))
bip_probs <- prop.part.df(trees)
ctree <- consensus(trees, p = 0.5); ctree$edge.length <- runif(length(ctree$edge[,1]), 2,3)
ctree <- midpoint.root(ctree)
plot(ctree)
multifurcating_nodes <- as.integer(attr(which(table(ctree$edge[,1]) > 2), "names"))
ctree$tip.label[getDescendants(ctree, node = multifurcating_nodes[2])]

#find biparts where group is a subset of the descendants of a bifurcating node, and resolve highest prob bipart
#be sure to start with the most inclusive multifurcation first