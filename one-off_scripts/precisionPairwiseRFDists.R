  ...
  for (k in 1:length(degreeMisspecification)) {
    ...
    for (j in 1:7) { #iterates over trait number
      ...
      for(i in m:m){ #iterates over replicates
        ...
        PairwiseDists[rowCounter, 2,] <- rep(rateMatrixSpecification, 1000)
...
colnames(means) <- c("ntraits", "rateMatrixSpecification", "replicateNum", "rfDist", "kfDist")
...
legend(54, 54,levels(means$rateMatrixSpecification),col=1:length(means$rateMatrixSpecification),pch=1)
...
legend(54, 3.225,levels(means$rateMatrixSpecification),col=1:length(means$rateMatrixSpecification),pch=1)
