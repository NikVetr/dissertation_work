...
foreach(m=1:100, .packages = c("ape", "coda", "rwty", "phangorn", "lattice", "latticeExtra", "MCMCpack", "readtext", "StatMatch", "mvtnorm")) %dopar% {
  ...
  # degreeSpecification <- c("2", "3", "4", "6", "8", "9")
  meanValueCodingDo <- F; avgRFdistMVC <- NA
  numStartingTrees <- 40 #number of independent random starting trees
  ...
  colnames(mpRFdists) <- c("ntraits", "rateMatrixSpecification", "replicateNum", "meanRF.divergenceCoding", "meanRF.meanValueCoding")
  
  # for (k in 1:1) {
  for (k in 1:length(degreeMisspecification)) {
  # for (k in 1:length(degreeSpecification)) {
    ...
    for (j in 1:length(traitNumIter)) { #iterates over trait number
      ...
      for(i in m:m){ #iterates over replicates
        ...
        # rateMatrixSpecification <- degreeSpecification[k]
        replicateNum <- i
        fileName <- paste0("simulations_", ntraits, "traits_", rateMatrixSpecification, "RateMatrix_", "replicate_", replicateNum)
        # fileName <- paste0("simulations_", ntraits, "traits_perfectRateMatrix_", rateMatrixSpecification, "SpecificationStrength_", "replicate_", replicateNum)
        ...
        DCT.pD <- phyDat(divergenceCodedTraits, type = "USER", levels = 1:(numPops*2-2))
        
        #use the parsimony ratchet to search for most parsimonious trees a number of times
        mpTrees <- list()
        for(sepStart in 1:numStartingTrees){
          mpTree <- pratchet(DCT.pD, maxit = 1e5, method = "sankoff", k = unchangedRun, start = rtree(n = length(tips), tip.label = sample(tips)), trace = traceOutput, all = T, cost = wagnerCostMatrix(numPops*2-2))
          mpTreeScore <- parsimony(tree = mpTree, data = DCT.pD, method = "sankoff", cost = wagnerCostMatrix(numPops*2-2))
        ...
        } else {avgRFdistMVC <- NA}
          
        #write to table
        mpRFdists <- rbind(mpRFdists, c(ntraits, rateMatrixSpecification, replicateNum, avgRFdistDC, avgRFdistMVC))
        print(paste0(ntraits, "traits, ", rateMatrixSpecification, " corr, ", avgRFdistDC, " divergence RF, ", avgRFdistMVC, " mean value RF"))
