...
for(rep_i in nreps){
  ...
  write.table(root_state, paste0("root_states/root_for_rep_", rep_i))
  
  traits <- mvSIM(tree, model="BM1", param=list(sigma=R, theta=root_state)) 
  write.table(traits, file = paste0("means/true_means_", rep_i))
  ...
  names(constraintArray) <- names(traits_indiv_discr)
  
  save(traits_indiv_discr, file = paste0("traits_indiv_discr/traits_indiv_discr_", rep_i))
  save(obs, file = paste0("obs/obs_", rep_i))
  save(constraintArray, file = paste0("constraint_arrays/constraintArray_", rep_i))
}
