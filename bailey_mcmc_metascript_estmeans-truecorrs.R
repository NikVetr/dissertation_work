...
n_optim_reps <- 4
params <- lapply(1:n_optim_reps, function(x) x)
for(run_iter in 1:4){
  load(file = paste0("data_sim/param_estimates_replicate_", rep_i, "_optimrep_", run_iter))
  params[[run_iter]] <- current_params
}

means <- params[[1]]$means
for(i in 2:n_optim_reps){
  means <- means + params[[i]]$means
}
means <- means / n_optim_reps
nTaxa <- nrow(means)
n_traits <- ncol(means)

corrMat <- as.matrix(read.table(paste0("output/nearPD_Corr_noPanAsian.txt")))
weight <- 0.02
...
curr_ll
BMpruneLLuvtChol(means, sig = diag(sqrt(curr_rates)) %*% curr_Rm %*% diag(sqrt(curr_rates)), tree = curr_tree)
BMpruneLLmvt(means, diag(sqrt(curr_rates)) %*% curr_Rm %*% diag(sqrt(curr_rates)), curr_tree)
...
burnin_prop = 0.3
proposal_probs <- c(2,8,1,2,0.5) #c("topology", "rates", "TL", "BLs", "rate_reg)
...
for(i in 2:n_iter){
  ...
  if(type_of_move == "BLs"){
    ...
    if(useSimplexSlideMoves){
      prop_bl_info <- slideMove(curr_relative_bls[which_BL], window = 0.075, min = 0, max = 1)
      prop_relative_bls <- curr_relative_bls
  ...
  if(type_of_move == "rates"){
    ...
    if(useSimplexSlideMoves){
      prop_relative_rates_info <- slideMove(curr_relative_rates[which_Rate], window = 0.025, min = 0, max = 1)
      prop_relative_rates <- curr_relative_rates
...
write.tree(phy = trees, file = paste0("output_sim/fixCorrs_infRates_allTraits_trees_regRates_replicate_estMuTrueCorr_", rep_i, "_c", mcmc_rep_i, ".trees"))
write.table(rates, file = paste0("output_sim/fixCorrs_infRates_allTraits_rates_regRates_replicate_estMuTrueCorr_", rep_i, "_c", mcmc_rep_i, ".txt"))
write.table(lls, file = paste0("output_sim/fixCorrs_infRates_allTraits_lls_regRates_replicate_estMuTrueCorr_", rep_i, "_c", mcmc_rep_i, ".txt"))
write.table(rate_regs, file = paste0("output_sim/fixCorrs_infRates_allTraits_rateRegs_regRates_replicate_estMuTrueCorr_", rep_i, "_c", mcmc_rep_i, ".txt"))
